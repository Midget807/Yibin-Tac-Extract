...
execute as @a[team=green] if score #canDeploy canDeploy matches 1 unless score @s deployCoolDown > #0 constants unless score @s inMap matches 1 if score @s inDeployBox matches 1 run scoreboard players set @s isDeploying 1
scoreboard players set #green isDeploying 0
execute as @a[team=green] if score #canDeploy canDeploy matches 1 unless score @s deployCoolDown > #0 constants unless score @s inMap matches 1 if score @s isDeploying matches 1 run scoreboard players set #green isDeploying 1

#warn if on deploy cooldown
execute as @a[team=green] if score #canDeploy canDeploy matches 1 if score @s deployCoolDown > #0 constants unless score @s inMap matches 1 if score @s inDeployBox matches 1 if score #shittyrng shittyrng matches 1 run title @s actionbar ["",{"text":"You may deploy again in","color":"gold"}," ",{"score":{"name":"@s","objective":"deployCoolDown"},"bold":true,"color":"yellow"}," ",{"text":"seconds.","color":"gold"}]

#warn if candeploy is zero
execute as @a[team=green] unless score #canDeploy canDeploy matches 1 unless score @s inMap matches 1 if score @s inDeployBox matches 1 if score #shittyrng shittyrng matches 1 run title @s actionbar {"text":"You cannot deploy right now.","color":"gold"}

#throw error if player's inmap is 1 
execute as @a[team=green] if score @s inMap matches 1 if score @s inDeployBox matches 1 if score #shittyrng shittyrng matches 1 run tellraw @s {"text":"ERROR: CANNOT DEPLOY - your inMap value is 1.","color":"red"}
...
execute as @a[team=green, tag=!UnableToTp] if score @s inDeployBox matches 1 if score #green deploySeconds matches 0 run tag @s add sfxbugfix
execute as @a[team=green, tag=!UnableToTp] if score @s inDeployBox matches 1 if score #green deploySeconds matches 0 run schedule function yibextract:inmap/sfxbugfix 2t
...
execute as @a[team=green, tag=!UnableToTp] if score @s inDeployBox matches 1 if score #green deploySeconds matches 0 run scoreboard players add #shittyrng shittyrng 1
